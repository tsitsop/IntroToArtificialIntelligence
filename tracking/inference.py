...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # ghost eaten - send to jail
        if noisyDistance == None:
            all_possible[self.getJailPosition()] = 1
        else:
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                all_possible[p] = emissionModel[trueDistance]*self.beliefs[p]
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for old_pos in self.legalPositions:
            ...
            for new_pos, probability in new_pos_dist.items():
                all_possible[new_pos] += self.beliefs[old_pos]*probability
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for particle in distribution:
                true_distance = util.manhattanDistance(pacmanPosition, particle)
                distribution[particle] *= emissionModel[true_distance]
